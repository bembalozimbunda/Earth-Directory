...
if (!content.includes('African Red Void')) {
  ...
  // Add Button to toggle African Red Void
  const newButton = `
              <button
                onClick={() => setShowRedVoid(true)}
                className="flex items-center gap-2 px-3 py-1.5 bg-rose-500/10 border border-rose-500/30 hover:border-rose-500/80 rounded transition-all group"
              >
  ...
  console.log("Patched VoidPortal for African Red Void");
} else {
