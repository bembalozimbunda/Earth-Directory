...
def main():
    ...
    if not invalid_freqs:
        ...
        print(f"[WARN] Uncalibrated frequencies detected: {invalid_freqs}")
    ...
    if os.path.exists(integrity_ledger_file):
        try:
            ...
            if chain_valid and len(blocks) > 0:
                ...
                print("[WARN] Ledger chain verification incomplete or degraded.")
        except Exception as e:
            print(f"[WARN] Could not parse integrity_ledger.json: {e}")
    else:
        print("[INFO] integrity_ledger.json will be initialized on first audit sweep.")
